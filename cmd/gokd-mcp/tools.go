...
func registerTools(server *mcp.Server, s *srv) {
	...
	addToolMaybe(s, server, &mcp.Tool{Name: "walk_stacks_all", Description: "Iterate every thread (capped by max_threads, default 64, max 256), switching the engine's current thread for each, and return per-thread stacks (capped by max_frames, default 32, max 256). Optionally also fetches per-thread registers. Best-effort restores the original current thread on the way out. Use this for hang analysis."}, s.walkStacksAll)
	...
	addToolMaybe(s, server, &mcp.Tool{Name: "read_string", Description: "Read a NUL-terminated string at address. encoding is 'ansi'|'utf16le'|'auto' (default auto). max_bytes defaults to 256 and is hard-capped at 4096; truncated=true if no NUL was found within the budget."}, s.readString)
	addToolMaybe(s, server, &mcp.Tool{Name: "dump_memory", Description: "Return a kd-style hexdump (16 bytes per row, address + grouped hex + ASCII gutter). width is 1/2/4/8 bytes per group (default 1). Hard-capped at 4096 bytes; set physical=true for kernel-mode physical reads."}, s.dumpMemory)
	...
	addToolMaybe(s, server, &mcp.Tool{Name: "inspect_struct", Description: "Walk a typed object's fields at an address. type_name must be qualified as 'module!type' (e.g. 'nt!_EPROCESS'). recurse follows nested struct types up to depth 3 (default 0). Total fields hard-capped at 1024 across all levels; truncated indicates the cap was hit."}, s.inspectStruct)
	...
	addToolMaybe(s, server, &mcp.Tool{Name: "summarise_target", Description: "Narrative summary of the target: full get_session_state, the first 20 modules, up to 32 threads, and a one-liner preview of any last exception or bug check. Use this immediately after open_dump or attach_kernel to orient."}, s.summariseTarget)
